...
export default class GridScreen {
    ...
    updateFocus() {
        ...
        this.cards.forEach((card, index) => {
            if (index === this.currentFocusedItemIndex) {
                console.log("updateFocus() - updating focus to index", index);
                card.focus();
            }
        });
    }
     ...
     * Exits the application on LG TVs.
     */
    handleBack() {
        if (window.webOSSystem) {
            window.webOSSystem.close();
        } else {
            console.log('WebOS System API is not available. Unable to exit application.');
        }
    }
}